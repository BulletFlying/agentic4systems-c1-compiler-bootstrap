...
The default profile follows the AEC ISA specification: only the opcodes,
types, memory spaces, and special registers listed in the ISA document are
accepted for encoding.  The legacy TRACK_B_V1 profile (which includes
extended opcodes) is retained for compatibility research.
"""
...
# ---------------------------------------------------------------------------
# Default ISA profile opcode table (production default)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Legacy extended profile (includes additional opcodes — for compatibility)
# ---------------------------------------------------------------------------
...
def _instruction_encoding_type(opcode_name: str, dtype: str) -> str:
    # SHL encoding: PTX input `shl.b32` encodes as AEC `SHL.u32`.
