...
- `ephonic/Agentic4SystemSummerSchoolContest` is a read-only upstream fact source.
...
- Every completion report records both repositories' `main` SHAs.
...
The tree must be clean. The official remote is fetch-only by policy. The writable destination must resolve to the BulletFlying repository.
...
- Official and project repository SHA verification.
...
- Confirm official `main` did not change because of project operations.
