...
1. 官方仓库当前默认分支中的 `Track-C/C1-compiler/spec.md` 与 `scoring.md`。
2. 官方 `Track-B/spec.md` Appendix A 的 AEC Precise ISA Specification。
3. 官方 Track-B assembly/binary/testcase、manifest、generator 和 build metadata。
...
- `ephonic/Agentic4SystemSummerSchoolContest`：官方上游，只读事实源；当前基线提交为 `87a99a5c544c0d67282144fe2eda488f5049a287`。
- `BulletFlying/agentic4systems-c1-compiler-bootstrap`：本项目可写 public repo。

强制规则：

- 禁止向官方仓库推送 branch、commit、tag、release、PR 或 issue，除非用户明确逐次授权；
- 本地 `origin` 即使指向官方仓库，也只能 fetch；所有写入必须明确指向 `c1-public` 或 BulletFlying 仓库；
- 每轮结束必须分别核对官方与项目仓库的 `main` SHA；
...
1. 记录 `git status --short`、branch、HEAD、remotes、官方与项目 `main` SHA。
